## Body

Welding Defect Detection Dataset in YOLO format

Totally 2028 images, labeled and divided into training set, test set, validation set.

It is divided into three categories: Bad Weld, Good Weld, Defect.

"Bad Weld", "Good Weld", "Defect"

Electronic virtual products sold without return or exchange.

## Images

![img_01.jpg](img_01.jpg)
![img_02.jpg](img_02.jpg)
![img_03.jpg](img_03.jpg)
![img_04.jpg](img_04.jpg)
![img_05.jpg](img_05.jpg)
![img_06.jpg](img_06.jpg)

item_1046041034795

Here is a pay link on Stripe ( https://buy.stripe.com/3cs8yP7sY87d0vu9AB ). Please contact me lonlonago@foxmail.com after funding $89, and I will send you a complete data files , thank you!

![111.png](111.png)
